...
4. Destination constraints are checked diagnostically against the resulting configurations; they do not filter the contribution.
...
When the destination state has explicit constraints, the full incoming contribution is still added to destination state semantics.
Configurations outside the destination constraints remain in `SS` and are shown in **red** as constraint violations. They are not treated as exit zones.
...
1. **No incoming path**: No enabled sequence of initial/triggered/autonomous controller transitions reaches the state
2. **Strict actions discard everything**: Every incoming transition action is disabled in the currently reachable component configurations

**Example**: If every incoming transition to a state emits an action that is disabled in all currently reachable component configurations, the state has empty semantics and is marked with a gold/yellow border.

**How to fix**:
- Check that at least one incoming transition can actually contribute configurations
- Review guards and actions, especially strict action emissions that may discard all candidates
...
An exit zone entry occurs when:
1. A component machine has an enabled autonomous transition.
2. The transition source is compatible with current state semantics.
3. The editor records the source/target pair and then classifies it as **internal** or **boundary**.

### Exit Zone Structure

The exit-zone list, tooltips, and extended details expose:
- **Machine ID** and **target proposition**
- **Origin category** (CS-only, SS-only, or both)
- **Coverage status** (covered/uncovered/internal/orphan/not required)

The source/target component transition is shown explicitly (`m:S→T`).
...
2. Add that codomain to computed SS
3. If explicit constraints exist, use them diagnostically to mark violating rows in red
```
...
Incoming controller transitions are handled similarly:

```
1. Compute the incoming transition contribution under strict action semantics
2. Add that contribution to destination state semantics
3. If explicit constraints exist, mark incompatible rows as constraint violations
4. Do not reclassify those violations as exit zones
```
...
**Hover details:** Exit zones are displayed one per line; hover a line to see coverage/status details. For origin-category and coverage analysis, use **Show Extended Details...**.

**Note**: For detailed analysis (including origin category, expected guard, and coverage transitions), right-click the state dashboard and select **"Show Extended Details..."**.
